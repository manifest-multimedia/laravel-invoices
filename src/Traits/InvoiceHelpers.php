...
trait InvoiceHelpers
{
    ...
    public function headerImg(string $headerImg)
    {
    ...
    {
        $type = pathinfo($this->headerImg, PATHINFO_EXTENSION);
        $data = file_get_contents($this->headerImg);

        return 'data:image/' . $type . ';base64,' . base64_encode($data);
    }
    public function bgImage()
    {
        $type = pathinfo($this->bg, PATHINFO_EXTENSION);
        $data = file_get_contents($this->bg);
